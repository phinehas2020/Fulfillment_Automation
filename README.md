...
Quick setup
1) In Odoo, configure Shopify Fulfillment settings:
   - Shopify: shop domain, access token, webhook secret
   - Shippo: API token, shipper phone
   - Print Agent: API key (shared with the Pi), max attempts, lease seconds
   - Automation: auto-process toggle
